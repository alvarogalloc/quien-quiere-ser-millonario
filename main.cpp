...
namespace {
  ...
  auto poll_random_question(std::size_t level) -> question
  {
    ...
    // no ocupo un gran generador de números aleatorios
  ...
  auto show_scores_gui(std::uint8_t current_question) -> bool
  {
    ...
    for (std::uint8_t i = questions.size(); i > 0; i--) {
      constexpr static ImVec2 spacing {0, 10};
      ImGui::Dummy(spacing);
      prize_box(i, current_question);
    }
    std::string_view prompt_text = "Presiona 'enter' para continuar";
    ImGui::PushFont(g_font->font);
    const ImVec2 prompt_pos {
      ImGui::GetWindowWidth() - (ImGui::CalcTextSize(prompt_text.data()).x + 10.f),
      ImGui::GetWindowWidth() - 50.f};
  ...
  // draw 50:50 filter,
  // 64x64 with text '50:50' at the top right corner
  // draw a button
  ...
  void question_gui(question_state& state, const question& current_question)
  {
    ...
    auto wrapped_text = [max_text_width = prompt_size.x - 128](std::string str) {
      ...
      if (text_size.x > max_text_width) {
        ...
        auto middle_it = std::find_if(str.begin() + middle, str.end(), [](char c) {
...
// anonymous namespace is not needed, but it's a good practice to use it when you don't want to expose the symbols to the global scope
      ...
      */
        ...
        if (data.state == question_state::correct) {
          ...
          if (data.current_round == 15) {
            data.current_scene = scene::winner;
            break;
          }
          data.current_round++;
          data.current_question = poll_random_question(data.current_round);
          ...
          data.current_scene = scene::gameover;
        }
        break;
      }
      case scene::gameover: {
        if (end_screen_gui("Perdiste!")) {
          data = game_data {};
        }
        break;
      }
      case scene::winner: {
        if (end_screen_gui("¡Felicidades, ganaste!")) {
          data = game_data {};
        }
        break;
      }
